...
- get_contact_readback — SPEAKS the saved name (spelled out) and phone digits to the customer itself, so the digits are always in English. Do NOT say the name or the number in your own line — the customer has already heard them; your line is only the warm ask of whether both are right, in their language. If the customer corrects either, just call set_customer_contact with the fix — it re-speaks the corrected details to the customer itself, so do NOT call get_contact_readback again and do NOT say the name or number in your own line; only ask whether it is right now
