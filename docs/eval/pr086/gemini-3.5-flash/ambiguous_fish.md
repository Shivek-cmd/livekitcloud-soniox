...
**SIERRA:** We have the Fish Pakora and the Punjabi Fish Curry. Which one of those would you like to go with?
...
**SIERRA:** Awesome, I've got one Fish Pakora down for you. What else can I get started for you today?

**USER:** That's it for now.
> `record_additional_requests({"response": "no"})` → ADDITIONAL REQUESTS RECORDED: none. ⏎ SPICE DEFAULTED: Fish Pakora set to medium (no preference stated — use set_item_spice if the customer actually named a level). ⏎ ORDER NOW: 1 x Fish Pakora (Amritsari Fish Pakora) [medium]. total=$14.99 ⏎ GUIDE: acknowledge warmly in the customer's language, in your own words — do NOT re-ask about spice or allergies — then keep the order moving (pickup or delivery next if not set yet).
**SIERRA:** Alright, perfect. And will this order be for pickup or delivery today?
